-- DocFlow FULL Deployment SQL
-- This includes both BASELINE + all 9 features
-- Execute in Supabase SQL Editor in order

-- ============================================================================
-- BASELINE: MISSING CORE TABLE - student_documents
-- ============================================================================
-- This was missing from the baseline schema and must be created first
...
CREATE INDEX idx_student_documents_source_id ON public.student_documents(source_document_id);
CREATE INDEX idx_student_documents_created ON public.student_documents(created_at DESC);
...
CREATE POLICY "Students can create own documents" ON public.student_documents FOR INSERT
  WITH CHECK (student_id = auth.uid());

CREATE POLICY "Students can update own documents" ON public.student_documents FOR UPDATE
  USING (student_id = auth.uid())
  WITH CHECK (student_id = auth.uid());

CREATE POLICY "Admins can view submissions for their templates" ON public.student_documents FOR SELECT
  USING (
...
-- Insert default message templates (for the first admin user)
...
SELECT 
  (SELECT id FROM auth.users WHERE role = 'authenticated' LIMIT 1),
  template_name,
...
-- ============================================================================
-- DEPLOYMENT COMPLETE
-- All baseline + 9 features have been deployed to the database
-- ============================================================================
